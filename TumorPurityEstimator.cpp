...
double TumorPurityEstimator::estimateTumorPurity(){
   std::cerr << "estimating tumor purity ... ";
    ...
    try{
        ...
        // purity estimation model
        purity = -3.3454 * median + 14.7747 * iqr + 4.0344 * median * median + -13.7777 * median * iqr + -5.2434 * iqr * iqr + 0.3058;

        if(purity > 1.0){
            purity = 1.0;
        }else if(purity < 0.0){
            throw std::runtime_error("The value of purity exceeds the model's estimation range: " + std::to_string(purity));
        }
        ...
        std::cerr << "[ERROR] Failed to estimate tumor purity, set purity to 0.0" << std::endl;
        purity = 0.0;
    }
...
void TumorPurityEstimator::peakValleyFilter(std::vector<PurityData> &purityFeatureValueVec, int &germlineReadHpCountThreshold){
    ...
    while(vecFeatureIter != purityFeatureValueVec.end()){
        if((*vecFeatureIter).germlineReadHpCountInNorBam < germlineReadHpCountThreshold){
            ...
            // flag the position as not used for purity estimation
...
void TumorPurityEstimator::writePurityResult(double &purity, BoxPlotValue &plotValue, size_t &iteration_times, int &germlineReadHpCountThreshold){
    try{
        ...
        purityResult << "#==================================\n";
        purityResult << "# TUMOR PURITY ESTIMATION REPORT\n";
        purityResult << "#==================================\n";
        ...
        purityResult << "#==========Estimation result===========" << std::endl;
